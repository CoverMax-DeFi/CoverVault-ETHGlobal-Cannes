...
const deployedAddressesPath = path.join(__dirname, '../ignition/deployments/chain-5003/deployed_addresses.json');
...
async function verifyContract(config, retries = 3) {
  return new Promise((resolve, reject) => {
    ...
    const command = `npx hardhat verify --network mantleTestnet${contractFlag} ${config.address}${argsString}`;
    ...
    exec(command, { timeout: 120000 }, async (error, stdout, stderr) => {
      if (error) {
        if (stderr.includes('Already Verified') || stdout.includes('Already Verified') || 
          ...
          resolve();
        } else if ((stderr.includes('ECONNRESET') || stderr.includes('network request failed') || 
                   stderr.includes('timeout')) && retries > 0) {
...
async function verifyAllContracts() {
  console.log('🚀 Starting Mantle contract verification...\n');
  ...
  console.log('\n📋 Summary:');
  console.log('Check https://sepolia.mantlescan.xyz/ to view your verified contracts');
}
